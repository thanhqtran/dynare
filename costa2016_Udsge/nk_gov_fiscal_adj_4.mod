...
var Y IP IG C CR CNR G KP KG L LR LNR R W CM P PI PIW Q RB LAMBDAR LAMBDANR T tau_c tau_k tau_l TRANS B A Sm SG SIG STRANS Stau_c Stau_l Stau_k U;

varexo e e_m e_G e_IG e_TRANS e_tau_c e_tau_l e_tau_k;
parameters sigma phi alpha1 alpha2 alpha3 beta delta deltaG rhoa psi theta thetaW psiW rhoG rhoIG rhoTRANS rhotau_c rhotau_l rhotau_k rhom gammaG gammaIG gammaTRANS gammatau_c gammatau_l gammatau_k phiG phiIG phiTRANS phitau_c phitau_l phitau_k gammaR gammaPI gammaY tau_css tau_lss tau_kss phic omegaR Psi1 Psi2 chi;
...
Pss*CRss*((P+CR)*(1+tau_css)+tau_css*tau_c) + Pss*IPss*((P+IP) *(1+tau_css)+tau_css*tau_c) + (Bss/RBss)*(B-RB) = Wss*LRss *((W+LR)*(1-tau_lss)-tau_lss*tau_l)+Rss*KPss*((R+KP(-1)) *(1-tau_kss)-tau_kss*tau_k) + Bss*B(-1) + omegaR*TRANSss*TRANS;
// 5-Tobin’s Q
(Qss/beta)*Q = (1-delta)*Qss*Q(+1) + LAMBDARss*Rss*Uss *(1-tau_kss)*(LAMBDAR(+1)+R(+1)+U(+1)-(tau_kss/(1-tau_kss))*tau_k(+1))-LAMBDARss*Pss*Uss*Psi1*U(+1);
...
Y IP IG CR CNR C G KP KG LR LNR L R W U PI RB T B tau_c tau_k tau_l TRANS A;
nk_gov_res_adj4 = oo_.irfs;