    // Model 7a. Taxes. Non-announced permanent change in the consumption tax rate
    // Dynare code
    // File: model7a.mod
    // José L. Torres. University of Málaga (Spain)
    // fixed the figure commands
    ...
    tauk = 0.225;
    taul = 0.344;
    F = tauc*C + taul*W*L + tauk*(R-delta)*K;  // Initial value for F
    end;
     // Steady state
    steady;
    ...
    periods 0;
    // Change is the tax rate with respect to the final value
    values 0;
    end;
